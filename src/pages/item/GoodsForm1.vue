    ...
    watch: {
      ...
      'goods.categories': {
        ...
        handler(val) {
          ...
          let _this = this
          // 根据分类加载规格参数
          this.$http.get("/item/spec/params?cid=" + val[2].id)
            .then(resp => {
              _this.specifications = resp.data;
              _this.template = [];
              debugger
              _this.skuTemplate = [];
          ...
          /*    const arr1 = [];
              ...
              // 过滤出SKU属性
            /*  for (let i = 0; i < _this.specifications.length; i++) {
                const params = _this.specifications[i]//.params;
                let x = params.length;
